...
class Database():
    ...
    def __init__(self):
        self.connection = sqlite3.connect("C:\\Users\\koval\\QA-auto21\\become_qa_auto.db")
    ...
    def get_customer_by_id(self, id):
        query = f"SELECT id, name, address, city, postalCode, country FROM customers\
                    WHERE id = {id}"
        self.cursor.execute(query)
        record = self.cursor.fetchall()
        return record
    ...
    def insert_product(self, product_id, name, descroption, qnt):
        query = f"INSERT OR REPLACE INTO products (id, name, description, quantity) \
            VALUES ({product_id}, '{name}', '{descroption}', {qnt})"
        self.cursor.execute(query)
        self.connection.commit()
